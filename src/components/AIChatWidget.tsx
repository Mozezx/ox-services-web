...
const AIChatWidget: React.FC<AIChatWidgetProps> = ({
    ...
    // Toggle button text alternation
    useEffect(() => {
        if (isOpen) {
            setShowText(false);
            setHasOpened(true);
        ...
        // Se o usuário já interagiu/abriu o chat, não mostramos mais a mensagem alternada
        if (hasOpened) return;
    ...
    // Focus input when chat opens (only on desktop to prevent keyboard on mobile)
    useEffect(() => {
    ...
    return (
        <>
            ...
            <button
                ...
                className={`ai-chat-button fixed z-50 flex items-center justify-center rounded-full shadow-lg hover:shadow-xl transition-all duration-500 hover:scale-105
                    ${isOpen ? 'w-14' : (showText ? 'w-auto px-6' : 'w-14')} h-14 
                    bottom-6 left-6 md:bottom-[88px] md:right-6 md:left-auto`}
                style={{
                    background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
                    animation: 'none' // Mantém suave, removendo o pulse padrão para não conflitar
                }}
                aria-label={isOpen ? textData.chat.closeChat : textData.chat.openChat}
            >
                <div className={`flex items-center gap-2 overflow-hidden whitespace-nowrap transition-all duration-500`}>
                    {isOpen ? (
                        <svg key="close" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white" className="w-6 h-6 animate-fade-in">
                            <path fillRule="evenodd" d="M5.47 5.47a.75.75 0 011.06 0L12 10.94l5.47-5.47a.75.75 0 111.06 1.06L13.06 12l5.47 5.47a.75.75 0 11-1.06 1.06L12 13.06l-5.47 5.47a.75.75 0 01-1.06-1.06L10.94 12 5.47 6.53a.75.75 0 010-1.06z" clipRule="evenodd" />
                        </svg>
                    ) : (
                        <>
                            <svg key="chat" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white" className={`w-7 h-7 transition-all duration-500 ${showText ? 'scale-90 opacity-90' : 'scale-100 opacity-100'}`}>
                            ...
                            {showText && (
                                <span className="text-white text-sm font-semibold animate-fade-in truncate">
            ...
            {isOpen && (
                <div
                    className="ai-chat-window fixed bottom-20 left-4 right-4 sm:left-6 sm:right-auto sm:bottom-24 md:bottom-6 md:right-[92px] md:left-auto z-50 sm:w-[360px] h-[600px] sm:h-[500px] max-h-[calc(100dvh-100px)] sm:max-h-[calc(100vh-150px)] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden"
                    style={{
